...
Route::group(['middleware' => ['lang', /*'check_user'*/]], function () {
    ...
    Route::get('/manager_login8848', function (Request $request) {
        $get = $request->all();
        $param = env('ADMIN_CHECK_PARAM','checkLogin');
        if(empty($get[$param]) || $get[$param] !== env('ADMIN_CHECK','1234$@@!')){
            return '无登陆权限！';
        }
...
//    });
    ...
    Route::get('/api/front',function(Request $request){
        ...
        return [
            ...
            'site_name' => 'TDMock',
            'site_logo' => 'https://whaleexc.com/logo/logo.png',
            'show_download'=>true,
            'show_customer'=>true,
        ];
...
//<--LDH-->
    ...
    Route::get('api/iepn', function () {
        ...
        $currency = Setting::getValueByKey('platform_currenys');
        $dsp = Setting::getValueByKey('iepn_dsp');
        $minnum = Setting::getValueByKey('iepn_min_num');
        $fixed=Setting::getValueByKey('iepn_price_fixed');
        // var_dump($fixed)
        ...
        $obj = ['type' => 'ok', 'message' => ['fixed'=>$fixed==1,'dsp'=>$dsp,'min'=>$minnum,'currency'=>$currency,'total' => $total, 'yfx' => $yfx, 'sy' => $total - $yfx, 'price' => $price, 'start' => date('Y-m-d H:i:s', strtotime($starttime)), 'end' => date('Y-m-d H:i:s', strtotime($endtime))]];
...
//Route::any('api/currency/new_timeshar', 'Api\CurrencyController@newTimeshars')->middleware(['cross']); //K线分时数据，对接tradeingview
    ...
    Route::get('/api/getService',function(){
        $settingList = st::whereIn('key', ["custorm_url_pc","custorm_url_mobile","other_service_url",'android_direct','ios_direct'])->get()->toArray();
        return ['type'=>'ok','message'=>array_column($settingList,'value','key')];
    });
...
//     Route::get('/bank','Api\UserController@bankList');//获取收款方式
        ...
        Route::post('wallet/real_nameh', 'Api\UserController@walletRealNameHigh');
        ...
        Route::group(['prefix' => 'bztl'], function () {
            Route::get('/getProfitList', 'Api\BztlController@getProfitList');//理财产品列表
            //持币生息
            Route::get('/getProfitTotal', 'Api\BztlController@getProfitTotal'); //获取资产信息
             //获取产品列表
            Route::post('/buyProfit', 'Api\BztlController@buyProfit'); //购买理财产品
            Route::post('/sellProfit', 'Api\BztlController@sellProfit'); //赎回理财产品
            Route::get('/getMyprofit', 'Api\BztlController@getMyprofit'); //我的理财产品
        });
    });
    ...
    Route::get('/admin/chargeCount',function(){
        ...
        $q = \App\MicroOrder::where('is_chakan','0')->count(); //是否已经查看

        echo json_encode(['ok'=>1,'message'=>['q'=>$q,'c'=>$c,'c1'=>$c1,'w'=>$w,'w1'=>$w1,'r'=>$r,'u'=>$u]]);
    });
    Route::get('new/walletList', 'Api\WalletOneController@walletList');//
    ...
    //自动生成数据
    Route::post('/admin/autoproduce', 'Admin\DefaultController@autoproduce');
...
//管理后台
    Route::group(['prefix' => 'admin', 'middleware' => ['admin_auth']], function () {
        ...
        Route::get('/statistic/index','Admin\StatisticController@index');
        Route::get('/statistic/lists','Admin\StatisticController@lists');
        Route::get('/statistic/lists2','Admin\StatisticController@lists2');
        ...
        //搬砖套利管理
        Route::get('/bztl/product', 'Admin\BztlController@index');//搬砖套利管理
        Route::get('/bztl/product/list', 'Admin\BztlController@wealthLists');//搬砖套利产品列表
        Route::get('/bztl/product/add', 'Admin\BztlController@add');//搬砖套利增加产品
        Route::post('/bztl/product/add', 'Admin\BztlController@doadd');//搬砖套利增加产品
        Route::get('/bztl/product/edit', 'Admin\BztlController@edit');//搬砖套利修改产品
        Route::post('/bztl/product/edit', 'Admin\BztlController@doedit');//搬砖套利修改产品
        Route::post('/bztl/product/del', 'Admin\BztlController@del');//搬砖套利删除产品
        //搬砖套利管理
        Route::get('/bztl/trade', 'Admin\BztlTransactionController@index');//搬砖套利交易列表
        Route::get('/bztl/trade/lists', 'Admin\BztlTransactionController@lists');//搬砖套利交易列表
