                ...
                window.onload = function() {
                    ...
                    layui.use(['element', 'form', 'layer', 'table'], function () {
                        ...
                        form.on('submit(mobile_search)',function(obj){
                            ...
                            tbRend("<?php echo e(url('/admin/user/hreal_list')); ?>?account="+account+"&review_status_s="+review_status_s);
                            return false;
                        });
                        function tbRend(url) {
                            table.render({
                                ...
                                , cols: [[
                                    ...
                                    , {field:'passport_id',title: '护照号',width: 150}
                                    , {field:'jiashi_id',title: '驾驶证号',width: 150}
                        ...
                        tbRend("<?php echo e(url('/admin/user/hreal_list')); ?>");
                        ...
                        form.on('switch(sexDemo)', function(obj){
                            ...
                            $.ajax({
                                url:'<?php echo e(url('admin/user/hreal_auth')); ?>',
                                type:'post',
                                dataType:'json',
                                data:{id:id},
                        ...
                        table.on('tool(userlist)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                            ...
                            if (layEvent === 'delete') { //删除
                                layer.confirm('真的要删除吗？', function (index) {
                                    //向服务端发送删除指令
                                    $.ajax({
                                        url: "<?php echo e(url('admin/user/hreal_del')); ?>",
                                        type: 'post',
                                        dataType: 'json',
                                        data: {id: data.id},
                                        success: function (res) {
                                ...
                                layer_show('认证详情','<?php echo e(url('admin/user/hreal_info')); ?>?id='+data.id);
                            }
                        });
                    });
                }
