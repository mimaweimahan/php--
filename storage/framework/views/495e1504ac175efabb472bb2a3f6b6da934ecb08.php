...
<?php $__env->startSection('page-content'); ?>
    <div style="margin-top: 10px;width: 100%;margin-left: 10px;">
        ...
        <button class="layui-btn layui-btn-normal layui-btn-radius"
                onclick="layer_show('添加机器人','<?php echo e(url('admin/robote/add')); ?>')">添加机器人
        </button>
    ...
    <script type="text/html" id="switchTpl">
        <input type="checkbox" name="status" value="{{d.id}}" lay-skin="switch" lay-text="是|否" lay-filter="sexDemo" {{
               d.status== 1 ? 'checked' : '' }}>
    </script>
...
<?php $__env->startSection('scripts'); ?>
    <script>
        ...
        layui.use(['table', 'form'], function () {
            ...
            table.render({
                elem: '#demo'
                , url: '<?php echo e(url('admin/robote/list_data')); ?>' //数据接口
                , page: true //开启分页
                , id: 'mobileSearch'
                , cols: [[ //表头
                    ...
                    , {field: 'min', title: '下浮动', minWidth: 40}
                    , {field: 'max', title: '上浮动', minWidth: 40}
                    , {field: 'second', title: '频率(秒)', minWidth: 60}
                    , {field: 'mult', title: '挂单', minWidth: 60}
            ...
            form.on('switch(sexDemo)', function (obj) {
                var id = this.value;
                $.ajax({
                    url: '<?php echo e(url('admin/robote/start')); ?>',
                    type: 'post',
                    dataType: 'json',
                    data: {id: id},
            ...
            table.on('tool(test)', function (obj) {
                var data = obj.data;
                if (obj.event === 'del') {
                    layer.confirm('真的删除机器人么', function (index) {
                        $.ajax({
                            url: '<?php echo e(url('admin/robote/delete')); ?>',
                            type: 'post',
                            dataType: 'json',
                            data: {id: data.id},
                            success: function (res) {
                    ...
                    layer_show('编辑机器人', '<?php echo e(url('admin/robote/add')); ?>?id=' + data.id);
                    ...
                    layer_show('预设的行情拐点', '<?php echo e(url('admin/robote/sche')); ?>?rid=' + data.id);
                }
            });
