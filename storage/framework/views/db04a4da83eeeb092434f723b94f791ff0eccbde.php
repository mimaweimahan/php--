        ...
        <div class="layui-inline" style="margin-left: 50px;">
            ...
            <div class="layui-input-inline">
                <select name="type" id="type" class="layui-input">
                    ...
                    <?php $__currentLoopData = $ins_type; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=> $ins): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    <script>
        layui.use(['table','form','laydate'], function(){
            ...
            table.render({
                elem: '#demo'
                ,toolbar: '#toolbar'
                ,url: '<?php echo e(url('admin/claim_list')); ?>' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'affirm'){
                    layer.confirm('确认理赔吗', function(index){
                        $.ajax({
                            url:'<?php echo e(url('admin/claim_affirm')); ?>',
                            type:'post',
                            dataType:'json',
                            data:{id:data.id},
                    ...
                    layer.confirm('是否驳回', function(index){
                        $.ajax({
                            url:'<?php echo e(url('admin/claim_reject')); ?>',
                            type:'post',
                            dataType:'json',
                            data:{id:data.id},
                            success:function (res) {
