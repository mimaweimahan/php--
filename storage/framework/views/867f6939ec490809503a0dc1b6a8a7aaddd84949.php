...
<?php $__env->startSection('scripts'); ?>
    ...
    <script>
        ...
        layui.use(['table','form'], function() {
            ...
            table.render({
                elem: '#demo'
                ,url: '<?php echo e(url('admin/insurance/lists')); ?>' //数据接口
                ,page: true //开启分页
                ,height: 'full-100'
                ,id: 'mobileSearch'
                ,cols: [[ //表头
            ...
            form.on('switch(status)', function(obj){
                var id = this.value;
                $.ajax({
                    url:'<?php echo e(url('admin/insurance/change_status')); ?>',
                    type:'post',
                    dataType:'json',
                    data:{id:id},
            ...
            form.on('switch(auto_claim)', function(obj){
                var id = this.value;
                $.ajax({
                    url:'<?php echo e(url('admin/insurance/change_auto_claim')); ?>',
                    type:'post',
                    dataType:'json',
                    data:{id:id},
                    success:function (res) {
            ...
            form.on('switch(is_t_add_1)', function(obj){
                var id = this.value;
                $.ajax({
                    url:'<?php echo e(url('admin/insurance/change_t_add_1')); ?>',
                    type:'post',
                    dataType:'json',
                    data:{id:id},
                    success:function (res) {
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('真的删除行么?可能会导致系统崩溃！', function(index){
                        $.ajax({
                            url:'<?php echo e(url('admin/insurance/del')); ?>',
                            type:'post',
                            dataType:'json',
                            data:{id:data.id},
                            success:function (res) {
                    ...
                    layer_show('编辑险种','<?php echo e(url('admin/insurance/add')); ?>?id='+data.id, 800, 600);
                }
            });
