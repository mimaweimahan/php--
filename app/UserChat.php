...
class UserChat extends Model
{
    ...
    protected static $_socket_client1 = null;
    ...
    public static function sendText($data)
    {
        ...
        try{
            ...
            $client1=self::getSocket1();
            
            $data1 = (is_scalar($data) ? strval($data) : json_encode($data));
            $data1=pack('N',strlen($data1)).$data1;
            
            fwrite($client1, $data1);
    ...
    public static function getSocket1()
    {
        self::getConfig();
        if (self::$_socket_client1 == null) {
            self::$_socket_client1 = stream_socket_client('tcp://' . self::$config['worker_push_url'] . ':' . 2070, $errno, $errstr);
        }
        if (!self::$_socket_client1) {
            throw new \Exception($errstr, $errno);
        }
        return self::$_socket_client1;
    }
