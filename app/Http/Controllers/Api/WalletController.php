...
//        var_dump($currency_match);
        if ($currency && $currency_match) {
            ...
            $price = $fixed==1?Setting::getValueByKey('iepn_price'):$price;
    ...
    {
         $user_id = Users::getUserId();
        $ieo = IeoRecord::orderBy('created_at','desc')->where('user_id', $user_id)->limit(100)->get();
        return $ieo;
    }
    //删除提币地址
    ...
    public function chargeReq(Request $request)
    {
        ...
        $acc = Input::get("acc", 'empty');
        $amount = Input::get("amount", 0);
        $hash = Input::get('hash', 0);

        if(empty($amount))
        {
            return $this->error('Please type the amount');
        }
...
//资产划转
    ...
    public function postWalletOut()
    {
        ...
        $user_id = Users::getUserId();
        $scurrency_id = Input::get("currency", '');
        $number = Input::get("number", '');
        $rate = Input::get("rate", '');
        ...
        try {
            ...
            $walletOut->currency = $scurrency_id;
            $walletOut->number = $number;
            $walletOut->address = $saddress;
            $walletOut->rate = $rate;
