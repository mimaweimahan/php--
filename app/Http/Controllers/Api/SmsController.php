...
//        die;
    ...
    public function sendMail(Request $request)
    {
        ...
        //var_dump($username,$host,$password,$mode,$port,$mail_from_name);die;
//        $mail_from_name='BSDW';
        //实例化phpMailer
//var_dump($username);
        try {
            ...
            // dd($mail_from_name);
            $mail->setFrom($mail_from_name,'Blockchain');//设置邮件来源  //发件人
            // $mail->Subject = $this->lang('mailTitle'); //邮件标题
            // dd($this->lang('mailTitle'));
            $mail->Subject = 'Blockchain, email authentication';
            $code = $this->createSmsCode(6);
            // dd($this->lang('codeTemplate', ['code' => $code]));
            $msg = 'Your verification code is: '.$code.', please enter it within 10 minutes';
            // $mail->MsgHTML($this->lang('codeTemplate', ['code' => $code]));   //邮件内容
            $mail->MsgHTML($msg);   //邮件内容
...
////            $smtp->debug = false;//是否显示发送的调试信息
            ...
            //intval($mode) === 1 ? $mail->send() : $smtp->sendmail($smtpemailto, $smtpusermail, $mailtitle, $mailcontent, $mailtype);
            if ($state) {
                session(['code' => $code]);
