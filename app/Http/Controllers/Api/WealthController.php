...
class WealthController extends Controller
{
    public function getMyprofit(){
        ...
        $query = WealthTransaction::where(function ($query) use ($status){
            if($status >0) {
...
//        }
    ...
    public function sellProfit(){
        ...
        $wealthTrade  = WealthTransaction::where('user_id',$user_id)->where('id',$id)->first();
        ...
        try{
            ...
            $breach =  $wealthTrade->caution_money - ($wealthTrade->reneged/100 * $shengyu_days *  $wealthTrade->caution_money);

            $wealthTrade->status = WealthTransaction::STATUS2;
            $wealthTrade->save();

            $user_change->change_balance = bc_add($user_change->change_balance, $breach, 5);
            $user_change->lock_wealth_balance = bc_sub($user_change->lock_wealth_balance, $wealthTrade->caution_money, 5);
            $user_change->save();
            DB::commit();
    ...
    public function buyProfit(){
        ...
        $pinfo = WealthProduct::where('id',$id)->first();
        if(empty($pinfo)){
        ...
        $exists = WealthTransaction::where("user_id", $user_id)->where('wealth_product_id',$id)->count();
        ...
        try{
            ...
            $now = time();
            $order = new WealthTransaction();
            $order->user_id = $user_id;
            $order->wealth_product_id = $id;
...
//            $order->reneged = $pinfo->reneged;
            ...
            $order->status	 = WealthTransaction::STATUS1;
            $order->expire_time	 = $now+(($pinfo->period+1)*86400);
    ...
    public function getProfitTotal(Request $request){
        ...
        $wealthTrade = WealthTransaction::where('user_id',$user_id)->where('status',1)->get();
        $totalRevenue = 0;// 总收益
        $dayRevenue = 0; //日收益
        ...
        $pname = '';
        if(!empty($pid)){
            $winfo = WealthProduct::where('id',$pid)->first();
            if($winfo){
    ...
    public function getProfitList(){
        $wealthProduct = WealthProduct::where('status',1)->get();
        $list = [];
        if($wealthProduct){
