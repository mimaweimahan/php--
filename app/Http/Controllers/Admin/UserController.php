...
//            }
    ...
    public function doedit()
    {
        ...
        $user_remark = Input::get("user_remark");
...
//        $user = Users::where('account_number',$request->account)
        $account = $request->get('account', '');
//        var_dump($account);
        if ($account) {
...
//        var_dump($obj);
        ...
        foreach ($obj as $n) {
            $exists = MyQuotation::where('base', $currency)->where('itime', strtotime($n[0]))->first();
            $needle = $exists ? $exists : new MyQuotation();
            $needle->open = $n[1];
            $needle->high = $n[2];
            $needle->low = $n[3];
            $needle->close = $n[4];
...
//            $needle->target = 'USDT';
//            $needle->symbol = "{$needle->base}/{$needle->target}";
//            $needle->itime = strtotime($obj[$i][0]);
//
//            $needle->save();
//        }
