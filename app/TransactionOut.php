...
class TransactionOut extends Model
{
    ...
    protected $appends = ['account_number', 'currency_name', 'legal_name','type'];
     ...
     * @return void
     */
    public static function transaction($in, $num, $user, $wallet, $legal_id, $currency_id, $match)
    {
         ...
         * 卖的直接成交 扣除卖方交易币
         */
        $fee = bc_mul($num * $in->price , ($match->change_trade_fee*0.01),5);
        ...
        $data_wallet1['after'] = bc_sub($wallet->change_balance, $num, 8);
        //扣除卖方的币
        $out_currency->change_balance = bc_sub($wallet->change_balance, $num, 8);
        $out_currency->save();
        ...
        $data_wallet3['change'] = -($cny + $fee);
        $data_wallet3['after'] = bc_sub($to_legal->lock_change_balance, ($cny + $fee), 5);
        //扣除买方的法币
        $to_legal->lock_change_balance = bc_sub($to_legal->lock_change_balance, ($cny + $fee), 5);
        $to_legal->save();
        ...
        $data_wallet4 = [
            ...
            'change' => $cny - $fee,
            'after' => bc_add($out_legal->change_balance, $cny - $fee, 5),
        ];
        //增加卖方cny
        $out_legal->change_balance = bc_add($out_legal->change_balance, $cny - $fee, 5);
        $out_legal->save();
