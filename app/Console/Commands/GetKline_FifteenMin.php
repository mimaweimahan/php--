                        ...
                        if ($result['hits']) {
                            ...
                            if ($last['itime'] === Robot::getNowTime($peroid) / 1000 || $last['itime'] < Robot::getNowTime($peroid) / 1000) {
                                ...
                                if ($last['itime'] < Robot::getNowTime($peroid) / 1000) {
                                    $current = true;
                                    $last_time = strtotime('+15 min', $last['itime']);
                                } else {
                                    $last_time = $last['itime'];
                                }
                                $start = $this->getInfo($last_time, strtotime('+15 min', $last_time), $currency_name);
...
//                            }
                    ...
                    if ($result === false) {
                        ...
                        if (isset($result_quotation['hits'])) {
                            ...
                            $time = $body['itime'];
                            $start = $this->getInfo($time, strtotime('+15 min', $time), $currency_name);
