...
class Updater extends Command
{
     ...
     * @var string
     */
    protected $signature = 'updater {id}';
     ...
     * @var string
     */
    protected $description = '匹配平台币更新器';
    ...
    {
        $this->info('--------------------------------------------------');
        $this->info('开始执行更新器:' . now()->toDateTimeString());
...
//            $currencys = CurrencyMatch::where('market_from', 3)->get()->toArray();
    ...
    protected function sendData($robot)
    {
        ...
        $price = MyQuotation::where('base', strtoupper($robot->currency_info))->where('itime', intval(self::getNowTime() / 1000))->first();
        $faker = Factory::create();
        if ($price) {
            $price = $price->toArray();

            $last_min_price = $faker->randomFloat($robot->mult, $price['close'] - $faker->randomFloat($robot->mult, $robot->float_number_down, $robot->float_number_up), $price['high']);
            $last_min_price = sprintf('%.' . $robot->mult . 'f', $last_min_price);
            $hh = json_encode(['ch' => $key, 'ts' => self::getNowTime(), 'tick' => [
                'id' => self::getNowTime() / 1000,
                'open' => $price['open'],
                'close' => $last_min_price,
                'low' => $price['low'],
                'high' => $price['high'],
                'vol' => $price['vol'],
                'amount' => $price['vol'],
        ...
        //五分钟线
        $price = MyQuotationSum::where('base', strtoupper($robot->currency_info))->where('ktype', '5min')->where('itime', intval(self::getNowTime('5min') / 1000))->first();
        ...
        if ($price) {
            ...
            $hh = json_encode(['ch' => $key, 'ts' => self::getNowTime(), 'tick' => [
                ...
                'open' => $price['open'],
                'close' => isset($last_min_price) ? $last_min_price : $price['close'],
                'low' => $price['low'],
                'high' => $price['high'],
                'vol' => $price['vol'],
                'amount' => $price['vol'],
        ...
        //15分钟线
        $price = MyQuotationSum::where('base', strtoupper($robot->currency_info))->where('ktype', '15min')->where('itime', intval(self::getNowTime('15min') / 1000))->first();
        ...
        //30分钟线
        $price = MyQuotationSum::where('base', strtoupper($robot->currency_info))->where('ktype', '30min')->where('itime', intval(self::getNowTime('30min') / 1000))->first();
        ...
        //一小时线
        $price = MyQuotationSum::where('base', strtoupper($robot->currency_info))
            ->where('ktype', '60min')->where('itime', intval(self::getNowTime('60min') / 1000))->first();
        ...
        //一天线
        $price = MyQuotationSum::where('base', strtoupper($robot->currency_info))
            ->where('ktype', '1day')->where('itime', intval(self::getNowTime('1day') / 1000))->first();
        ...
        //一周线
        $price = MyQuotationSum::where('base', strtoupper($robot->currency_info))
            ->where('ktype', '1week')->where('itime', intval(self::getNowTime('1week') / 1000))->first();
        ...
        //一月线
        $price = MyQuotationSum::where('base', strtoupper($robot->currency_info))
            ->where('ktype', '1mon')->where('itime', intval(self::getNowTime('1mon') / 1000))->first();
        $key = "market.{$currency_name}.kline.1mon";

        if ($price) {
            $price = $price->toArray();
            $hh = json_encode(['ch' => $key, 'ts' => self::getNowTime(), 'tick' => [
                'id' => self::getNowTime('1mon') / 1000,
                'open' => $price['open'],
                'close' => isset($last_min_price) ? $last_min_price : $price['close'],
                'low' => $price['low'],
                'high' => $price['high'],
...
//        $this->SaveQuotation($robot);
        ...
        //随机价格
        $price = $robot->currency_info;//$this->getPrice(strtolower($robot->currency_info . $robot->legal_info), $robot->float_number_down, $robot->float_number_up);
//        var_dump($price);
        $faker = Factory::create();
        $now_price = $price['close'];
        $now_price1 = $price['close'];
        $currency_name = strtolower($robot->currency_info . 'usdt');
        $key = "market.{$currency_name}.kline.1min";
